...
import {CreatorHubServiceManager} from "../../src/CreatorHubServiceManager.sol";
...
library CreatorHubDeploymentLib {
    using stdJson for *;
    using Strings for *;
    ...
    struct DeploymentData {
        address creatorHubServiceManager;
    ...
    function deployContracts(
        ...
        result.creatorHubServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        // Deploy the implementation contracts, using the proxy contracts as inputs
        address stakeRegistryImpl =
            address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address creatorHubServiceManagerImpl = address(
            new CreatorHubServiceManager(
                core.avsDirectory, result.stakeRegistry, core.rewardsCoordinator, core.delegationManager
            )
        );
        // Upgrade contracts
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (result.creatorHubServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);
        UpgradeableProxyLib.upgrade(result.creatorHubServiceManager, creatorHubServiceManagerImpl);
    ...
    function readDeploymentJson(
        ...
        data.creatorHubServiceManager = json.readAddress(".contracts.creatorHubServiceManager");
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.creatorHubServiceManager));
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            '","creatorHubServiceManager":"',
            data.creatorHubServiceManager.toHexString(),
            '","creatorHubServiceManagerImpl":"',
            data.creatorHubServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
